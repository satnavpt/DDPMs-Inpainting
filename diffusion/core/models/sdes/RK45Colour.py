...
def rk_step(fun, t, y, f, h, A, B, C, K, grey, shape, device, sde):
    ...
    mask = get_colourisation_mask(ytest)
    ...
    masked_data_mean, std = sde.marginal_prob(decouple(grey), vec_t)
    masked_data = masked_data_mean + torch.randn_like(y_out) * std[:, None, None, None]
    y_out = couple(decouple(y_out) * (1.0 - mask) + masked_data * mask)
